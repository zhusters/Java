...
public class task33 {
    public static void main(String[] args) {
        //Создадим массив зададий для учеников. Так как учеников 15,
        // каждый из них получит свою строку (задание).
        // Задание у нас состоит из 2 множителей и их произведения,
        // т.е. в каждой строке будет 3 элемента
        ...
        for(int i =0;i<Mas.length;i++){
            for(int h=0;h<Mas[i].length;h++){
                ...
                if(h==Mas[i].length-1){
                    ...
                    //Проверим, нет ли среди уже свормированых заданий (строк),
                    // таких, которые по условиям задачи нельзя задавать другому ученику.
                    // Для этого переберем все результаты умножения (3 элементы) предыдущих строк.
                    for(int q=0;q<i;q++){
                        if(Mas[i][h]==Mas[q][h]&&i>0){
                            //Если мы нашли такую строку, надо убедиться, что множители
                            // в ней тоже совпадают. Для этого нам достаточно проверить совпадение
                            // одного множителя.
